...
from cs336_basics.model import Embedding, Linear, MultiheadSelfAttention, RotaryPositionalEmbedding, SwiGLUFNN, RMSNorm, TransformerBlock, TransformerLM, load_checkpoint, save_checkpoint, scaled_dot_product_attention, softmax
...
def run_multihead_self_attention_with_rope(
    ...
    model = MultiheadSelfAttention(d_model, num_heads, rope=rope)
    model.q_proj.weight.data = q_proj_weight
    model.k_proj.weight.data = k_proj_weight
    model.v_proj.weight.data = v_proj_weight
    model.output_proj.weight.data = o_proj_weight
...
def run_load_checkpoint(
    ...
    """
    return load_checkpoint(src, model, optimizer, map_location="cpu")
...
def get_tokenizer(
    ...
    merges = {m : 256 + i for i, m in enumerate(merges)} # 256 + i since 256 vocab values are reserved for vocab init

    bpe = BPETokenizer.from_trained_tokenizer(vocab, merges, special_tokens)
